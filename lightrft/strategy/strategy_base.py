...
class StrategyBase(ABC):
    ...
    def convert_keys_to_string(self, obj):
        """
        Recursively convert all keys to strings in a dictionary or a list of dictionaries.
        """
        if isinstance(obj, dict):
            new_dict = {}
            for k, v in obj.items():
                new_key = str(k)
                new_dict[new_key] = self.convert_keys_to_string(v)
            return new_dict
        elif isinstance(obj, list):
            return [self.convert_keys_to_string(item) for item in obj]
        else:
            return obj
        ...
        if self.inference_engine_type == "vllm":
            ...
            return [
                EasyDict(
                    ...
                    # return logprobs to calculate difference from old_log_probs in fp8 rollout
                    rollout_log_probs=self.convert_keys_to_string(output.outputs[0].logprobs),
