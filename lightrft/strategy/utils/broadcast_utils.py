...
class BroadcastManager:
    ...
    def _fsdp_v2_broadcast(self):
        ...
        # if use fp8_rollout, vllm is in v1 mode, which has different load_weights logits
        if self.strategy.args.fp8_rollout:
            assert self.strategy.engine_type == "vllm", "Currently fp8 rollout only support vllm engine"
            ...
            vllm_model.load_weights((
                name, param.to(get_current_device(), non_blocking=True).full_tensor().
                to(torch.float8_e4m3fn) if isinstance(param, DTensor) else param.to(torch.float8_e4m3fn)
            ) for name, param in model.named_parameters())
        else:
            for name, param in model.named_parameters():
                count += 1  # empty_cache at last param
