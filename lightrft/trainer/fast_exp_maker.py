    ...
    def _build_unpacked_sample(
        ...
        **kwargs,
        ...
        for j, output in enumerate(outputs):
            ...
            # Calculate rollout log probs, which corrects log probabilities during vLLM inference.
            # It extracts the actual token probability from rollout data, converts to tensor, truncates
            # to max length, and transfers to GPU. This ensures alignment between generated tokens and their
            # probabilities for downstream training consistency.
            if self.strategy.args.enable_vllm_is_correction:
                rollout_log_probs = []
                truncate_length = max_new_tokens
                response_ids = list(output.output_token_ids)

                for k, logprob in enumerate(output.rollout_log_probs):
                    rollout_log_probs.append(logprob[str(response_ids[k])].logprob)
                rollout_log_probs = torch.tensor(rollout_log_probs)
                rollout_log_probs = rollout_log_probs[:truncate_length].to("cuda")
                all_rollout_log_probs.append(rollout_log_probs)

        if len(all_rollout_log_probs) > 0:
            final_rollout_log_probs = pad_sequence(all_rollout_log_probs, batch_first=True, padding_value=0.0)
        else:
            final_rollout_log_probs = torch.tensor([], device="cuda")
        ...
        if not is_multimodal:
            return Samples(
                ...
                references=references,
                pad_len=None,
                rollout_log_probs=final_rollout_log_probs if rollout_log_probs is not None else None,
