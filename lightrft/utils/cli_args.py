...
def add_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable float8 rollout, currently only support vllm engine",
    )
    parser.add_argument(
        "--enable_vllm_is_correction",
        action="store_true",
        default=False,
        help="Whether to enable vllm precision correction. When using fp8 rollout, should enable this feature.",
    )
    parser.add_argument(
        "--vllm_is_truncated_threshold",
        type=float,
        default=2,
        help="Upper bound of importance sampling weights.",
    )
