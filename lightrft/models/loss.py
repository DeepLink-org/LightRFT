...
class PolicyLoss(nn.Module):
    ...
    def forward(
        ...
        # https://fengyao.notion.site/off-policy-rl. This code implements importance
        # sampling​ to adjust the policy gradient loss. It calculates the ratio (vllm_is)
        # between the probabilities of the old and new policies for the actions taken,
        # which is then clipped to a threshold (vllm_is_truncated_threshold) to control
        # variance. This ratio scales the original loss, weighting each action's contribution
        # based on how much the policy has changed. Finally, the loss is averaged only over
        # the valid actions using the action mask.
        if rollout_log_probs is not None and self.vllm_is_truncated_threshold is not None:
