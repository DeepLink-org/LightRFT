...
LightRFT是一款轻量级的LLM模型后训练框架，用来支撑安全与多模态领域的后训练探索。
...
```bash
--rm_use_engine                   # 奖励模型使用推理引擎（vLLM/SGLang）
...
推荐：train_batch_size >= rollout_batch_size × n_samples_per_prompt
...
- 业务团队框架组：负责算法生态、训练策略、多模态支持及实验工具链的研发，专注于算法创新和模型训练能力的提升。
- 系统团队 DeepLink：负责高性能推理引擎、资源协同机制、系统级性能优化及底层基础设施，专注于系统性能和资源利用效率的优化。
...
**LightRFT**由上海人工智能实验室**安全可信中心强化学习团队**与**系统平台中心DeepLink团队**协作开发，感谢两个团队参与贡献的开发者。
- 本项目的强化学习部分基于 [OpenRLHF](https://github.com/OpenRLHF/OpenRLHF) 开发，我们向其开发团队的杰出工作表示衷心感谢。
- 我们感谢 Qwen、InternVL 和 DeepSeek 团队提供了优秀的开源基础模型。
- 我们感谢 DeepSpeed、PyTorch、vLLM 和 SGLang 等开源社区提供的强大工具。
